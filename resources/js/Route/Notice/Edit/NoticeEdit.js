...
export default class NoticeCreate extends Component {
    ...
    async handleSubmit(e){
        e.preventDefault();
        return await Axios.put(`/notices/${this.props.match.params.id}`, {
    ...
    async _getNotice(){
        return await Axios.get(`/notices/${this.props.match.params.id}`).then(response => (
            this.setState({
                title : response.data.notice.title,
                description :response.data.notice.description,
                thumbnail : response.data.notice.thumbnail
            })
        ))
    }
